...
import 'package:yacht_master_admin/src/dashboard/pages/approvals/view/widgets/approval_dialogue.dart';
...
import 'package:yacht_master_admin/src/dashboard/pages/approvals/view/widgets/approvals_grid.dart';
...
class ApprovalsDataGridSource extends DataGridSource {
  /// Creates the order data source class with required details.
  ApprovalsDataGridSource({this.model, required this.isWebOrDesktop}) {
  ...
  void buildDataGridRows(UserVM vm) {
    if (vm.selectedIndex == 3) {
      dataGridRows = tickets
          .where((element) =>
              (element.isPending!) &&
              element.charterFleetDetail!.name
                  .toString()
      ...
      dataGridRows = tickets
          .where((element) => element.charterFleetDetail!.name
              .toString()
              .isCaseInsensitiveContains(vm.searchController.text))
        ...
        Align(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color:
                  model.isPending! ? R.colors.yellowDark : R.colors.greenButton,
              borderRadius: BorderRadius.circular(6),
            ),
            child: Text(
              model.isPending!
                  ? LocalizationMap.getTranslatedValues("pending")
                  : LocalizationMap.getTranslatedValues("accepted"),
              style: R.textStyles.poppins(
        ...
        PopupMenuButton(
          ...
          onSelected: (val) async {
            ...
            requestDataGridSource =
                ApprovalsDataGridSource(isWebOrDesktop: isWebOrDesktop);
            Get.forceAppUpdate();
          },
          itemBuilder: (context) => model.isPending!
              ? <PopupMenuItem>[
  ...
  Future<void> updateApprovalsStatus(
    String id,
    bool status,
    BookingsVm vm,
  ) async {
    try {
      ...
      // });
      status
          ? await FBCollections.bookings.doc(id).delete()
          : await FBCollections.bookings.doc(id).update({"isPending": status});
      // await NotificationService.sendNotification(
      //     fcmToken: userFCM,
      //     title: "Acceptance to Recieve Payouts",
      //     body: desc);
